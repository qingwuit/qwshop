    ...
    <div class="qingwu">
        ...
        <div class="admin_form">
            <a-form-model :label-col="{ span: 5 }" :wrapper-col="{ span: 12 }">
                <a-form-model-item label="优惠券名称" :rules="{ required: true}">
                    <a-input v-model="info.name" />
                </a-form-model-item>
                <a-form-model-item label="优惠券额度" :rules="{ required: true}">
                ...
                <a-form-model-item label="发放总数" :rules="{ required: true}">
                    <a-input suffix="张" type="number" v-model="info.stock" />
                </a-form-model-item>
                <a-form-model-item label="优惠券描述" :rules="{ required: true}">
                    <a-textarea
                    placeholder="优惠券描述"
...
export default {
    ...
    methods: {
        ...
        handleSubmit(){
            ...
            let api = this.$apiHandle(this.$api.sellerCoupons,this.id);
            if(api.status){
        ...
        get_info(){
            this.$get(this.$api.sellerCoupons+'/'+this.id).then(res=>{
                this.info = res.data;
            })
        },
        // 获取列表
