<template>
    <div class="qwit">
        <table-view :handleWidth="'80px'" :options="options" :searchOption="searchOptions" :params="params" :btnConfig="btnConfigs" :dialogParam="dialogParam" ></table-view>
    </div>
</template>
...
export default {
    ...
    setup(props) {
        ...
        const options = reactive([
            {label:'优惠劵',value:'name'},
            {label:'额度',value:'money'},
            {label:'消费金额',value:'use_money'},
            {label:'剩余（张）',value:'stock',type:'tags'},
            {label:'优惠券状态',value:'status',type:'dict_tags'},
        ...
        const addColumn = [
            ...
            {label:'消费金额',value:'use_money',type:'number'},
            {label:'数量',value:'stock',type:'number'},
            {label:'开始时间',value:'start_time',type:'datetime'},
            {label:'结束时间',value:'end_time',type:'datetime'},
            {label:'描述',value:'content',type:'textarea',span:24},
        ]
