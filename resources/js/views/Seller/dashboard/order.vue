    ...
    <div class="qwit">
        ...
        <div class="admin_form plot" >
            <div class="sort_list">
                ...
                <div class="list_block" v-for="v in 10" :key="v"><span>{{v}}</span><em class="goods_name">{{data.store[v-1]?data.store[v-1]['goods_name']:'-'}}</em><em class="num">{{data.store[v-1]?data.store[v-1]['orders_count']:'-'}}</em></div>
            </div>
...
export default {
    setup(props) {
        ...
        const loadData = ()=>{
            proxy.R.get('/Seller/dashboard/order',params).then(res=>{
                if(!res.code){
                    // data.list = res.list
