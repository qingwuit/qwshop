            ...
            <a-raw :gutter="{ xs: 34,  md: 34}">
                <a-col :span="12" :xs="{ span: 24 }" :lg="{ span: 10 }">
                    <div class="sort_list">
                        ...
                        <div class="list_block" v-for="v in 10" :key="v"><font style="color:#999;float:right;">{{store[v-1]?store[v-1]['orders_count']:'-'}}</font><span>{{v}}</span>{{store[v-1]?store[v-1]['store_name']:'-'}}</div>
                    </div>
                </a-col>
...
export default {
    ...
    methods: {
        ...
        get_info(){
            this.$get(this.$api.adminStatistics+'/order',this.params).then(res=>{
