        ...
        <el-dialog custom-class="table_dialog_class" v-model="data.vis" title="订单处理">
            <div class="order_list" v-for="(v,k) in data.order" :key="k">
                ...
                <div class="order_goods">
                    <el-row :gutter="20" >
                        <el-col :span="4">
                            <el-image :style="{width:'30px',height:'30px'}" :fit="'cover'" :hide-on-click-modal="true" :src="v.order_image" lazy >
                                <template #error>
                                    <el-icon :color="'#888'" :size="26"><Picture /></el-icon>
                                </template>
                            </el-image>
                        </el-col>
                        <el-col :span="4">{{v.order_name||'-'}}</el-col>
                        <el-col :span="4">{{v.sku_name||'-'}}</el-col>
                        <el-col :span="4">{{$t('btn.money')}}{{v.total_price||'-'}}</el-col>
                        <el-col :span="4">x {{v.buy_num||'-'}}</el-col>
                        <el-col :span="4" style="color:red">{{$t('btn.money')}}{{v.total_price||'0.00'}}</el-col>
                    </el-row>
                </div>
...
export default {
    ...
    setup(props) {
        ...
        const editColumn = [
            ...
            {label:'地址信息',value:'receive_area'},
            {label:'详细地址',value:'receive_address'},
            {label:'快递公司',value:'delivery_code' ,type:'select',labelName:'name',valueName:'code'},
            {label:'快递订单号',value:'delivery_no'},
        ]
        ...
        const loadData = async ()=>{
            delivery() // 加载物流公司
            const res = await proxy.R.get('/Admin/integral_orders',{isAll:true,id:data.selected.join(',')+'|in'})
        ...
        const postDelivery = async ()=>{
            ...
            data.order.map(item=>{
                proxy.R.put('/Admin/integral_orders/'+item.id,{delivery_code:item.delivery_code,delivery_no:item.delivery_no}).then(()=>{
                }).finally(()=>{
                    sucNum++
