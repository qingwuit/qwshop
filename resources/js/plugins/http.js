...
axios.interceptors.response.use(function (res) {
    ...
    loginData.map((item,key)=>{
        if(res.config.url.indexOf(item.loginName) != -1 && item.tokenName != ''){
    ...
    if(res.data && (res.data.data || res.data.data === false || res.data.data === 0) && res.data.code == 200) return res.data.data;
    if(res.data && (res.data.data || res.data.data === false || res.data.data === 0) && res.data.code != 200){
        ElMessage.error(res.data.msg);
        return res.data
    }
	// 防止多次出现
    ...
    if (err.response.status) {
        switch(err.response.status){
            case 404:
                ...
                break;
            case 500:
                ElMessage.error('500 (Internal Server Error) '+ err.response.data.message);
            break;
            case 401:
                ...
                break;
            default:
                ElMessage.error(err.response.statusText+",error_code："+err.response.status);
            break;
        }
      
    }else{
        ElMessage.error("未知错误,错误信息："+err.response.statusText+"！");
    }
...
const R = {
   ...
   getLocalesName:()=>{
       ...
       isEmpty(locales)
       const language = !isEmpty(locales)?locales:(
           (navigator.language ? navigator.language : navigator.userLanguage) || "zh-cn"
       ).toLowerCase();
       return language
   },
