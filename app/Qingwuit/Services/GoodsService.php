...
class GoodsService extends BaseService{
    ...
    public function addGoods($auth = 'users'){
        ...
        $storeId = 0;
        if($auth == 'users') $storeId = $this->getService('Store')->getStoreId()['data'];
        $data = [
            'store_id'              => $storeId??0,
        ...
        if(!empty($storeConfig)){
            if($storeConfig['goods_verify']) $data['goods_verify'] = 0;
        }
    ...
    public function editGoods($goodsId,$auth = 'users'){
        $storeId = 0;
        if($auth == 'users') $storeId = $this->getService('Store')->getStoreId()['data'];
        $goodsModel = $this->getService('Goods',true)->where(['store_id'=>$storeId,'id'=>$goodsId])->first();

        if(!$goodsModel) return $this->formatError(__('tip.error'));
    ...
    public function getGoodsInfo($goodsId){
        ...
        if(!$goodsInfo) return $this->formatError(__('tip.error'));

        // 获取商品分类信息
        if($goodsInfo->class_id == 0) return $this->formatError(__('tip.error').'c');
        $classData = [];
            ...
            foreach($goods_attr as $k=>$v){
                foreach($v['specs'] as $key=>$vo){
                    if(in_array($vo['id'],$spec_id)){
                        ...
                        if(request()->saveCheck) unset($goods_attr[$k]['specs'][$key]);
                    }
                }
            }
