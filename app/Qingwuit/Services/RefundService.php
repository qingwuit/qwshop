...
class RefundService extends BaseService{
    ...
    public function add(){
        ...
        if(!$order_info) return $this->formatError(__('tip.error'));
        if($order_info->pay_time->lt(now()->subDays(7))) return $this->formatError('over 7 days');
        
        if($refund_model->where('user_id',$userId)->where('refund_verify',0)->where('order_id',$order_id)->exists()) return $this->formatError('refund handled');
    ...
    public function edit($id,$auth='user'){
        ...
        if(!$refund_info) return $this->formatError(__('tip.error'));

        if(isset(request()->refund_verify) && $auth =='seller'){
            $refund_verify = request()->refund_verify;
            if($refund_info->refund_verify!=0) return $this->formatError(__('tip.error').'- rep');
