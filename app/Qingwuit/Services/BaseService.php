...
class BaseService{
        ...
        if(!empty($requestAll)){
            foreach($requestAll as $k=>$v){
                if(!in_array($k,$this->whereExcept)){
                    ...
                    if(count($vArr) >= 2){
                        if($vArr[1] == 'like') $tableModel = $tableModel->where($k,'like','%'.$vArr[0].'%');
                        if($vArr[1] == 'likeRight') $tableModel = $tableModel->where($k,'like',$vArr[0].'%');
                        if($vArr[1] == 'likeLeft') $tableModel = $tableModel->where($k,'like','%'.$vArr[0]);
                        if($vArr[1] == 'gt') $tableModel = $tableModel->where($k,'>',$vArr[0]);
                        if($vArr[1] == 'lt') $tableModel = $tableModel->where($k,'<',$vArr[0]);
                        if($vArr[1] == 'ngt') $tableModel = $tableModel->where($k,'>=',$vArr[0]);
                        if($vArr[1] == 'nlt') $tableModel = $tableModel->where($k,'<=',$vArr[0]);
                        if($vArr[1] == 'in') $tableModel = $tableModel->whereIn($k,explode(',',$vArr[0]));
                    }else{
    ...
    public function addDat($tableModelName = null, $column = []){
        ...
        if($this->setUser) $data[$this->belongName] = $this->getBelongId($this->auth); // 是否根据所属ID来

        // 如果是密码之类的一律使用hash加密存储
        if(isset($data['password']) && !empty($data['password'])) $data['password'] = Hash::make($data['password']);
        if(isset($data['pay_password']) && !empty($data['pay_password'])) $data['pay_password'] = Hash::make($data['pay_password']);

        $rs = $tableModel->create($data);
        if(!$rs) return $this->formatError('insert data error.');
    ...
    public function editDat($tableModelName = null , $id = 0, $column = [] ,$where = [] , $keyName = 'id'){
        ...
        if(!$rs) return $this->formatError('edit data error.');
        return $this->format($rs);
    }
    ...
    public function findDat($tableModelName = null, $id = 0,  $where = [] ,$keyName = 'id'){
        ...
        // 判断是否存在资源文件
        $data = $rs;
        if(!empty(request('isResource'))) $tableModelName .=  request('isResource');
    ...
    public function setUserConfig($data = ['setUser'=>false,'auth'=>'admins','belongName'=>'belong_id']){
        if(isset($data['setUser'])) $this->setUser = $data['setUser'];
        if(isset($data['auth'])) $this->auth = $data['auth'];
        if(isset($data['belongName'])) $this->belongName = $data['belongName'];
        return $this;
    }
    ...
    public function getSuper($auth = 'admins'){
        try {
            $info = auth($auth)->user();
            if($auth != 'admins') return !empty($info['belong_id'])?false:true;
    ...
    public function getRoles($auth = 'admins',$with){
        try {
            $info = auth($auth)->user();
            $modelName = 'Admin';
            if($auth == 'admins') $modelName = 'Admin';
            if($auth == 'users') $modelName = 'User';
            ...
            if($roles){
                foreach($roles as $v){
                    if(isset($v['menus']) && !empty($v['menus'])) $rolesData['menus'] = array_merge($rolesData['menus'],$v['menus']);
                    if(isset($v['permissions']) && !empty($v['permissions'])) $rolesData['permissions'] = array_merge($rolesData['permissions'],$v['permissions']);
                    $rolesData['roleName'][] = $v['name'];
                }
                if(isset($rolesData['menus']) && !empty($rolesData['menus'])) $rolesData['menus'] = array_unique($rolesData['menus']);
                if(isset($rolesData['permissions']) && !empty($rolesData['permissions'])) $rolesData['permissions'] = array_unique($rolesData['permissions']);
            }
