...
class OrderService extends BaseService{
    ...
    public function createOrderBefore(){
        ...
        if(!$check['status']) return $this->formatError($check['msg']);
        $params = $check['data'];
    ...
    public function createOrder(){
        ...
        if(!$rs['status']) return $this->formatError($rs['msg']);

        // 优惠券的处理 
        if(!isset(request()->coupon_id)) return $this->formatError('coupon_id empty');
        $coupon_id = explode(',',request()->coupon_id);

        // 地址验证
        $address_resp = $this->checkAddress();
        if(!$address_resp['status']) return $this->formatError($address_resp['msg']);
            ...
            DB::beginTransaction();
                ...
                foreach($rs['data'] as $k=>$v){
                    ...
                    foreach($v['goods_list'] as $vo){
                        ...
                        if($seckillInfo['status']) $coupon_money += $order_goods_data['total_price']*($seckillInfo['data']['discount']/100);
                    ...
                    if($full_reduction_resp['status']) $coupon_money += $full_reduction_resp['data']['money'];
                    ...
                    if($collective_id != 0){
                        ...
                        if($collective_resp) $coupon_money += $order_price*($collective_resp['data']['discount']/100); // 得出拼团减去的钱
    ...
    public function orderStock($goods_id,$sku_id,$num,$is_type=0){
        ...
        try{
            ...
            if(empty($sku_id)){
                ...
                if(!$goods_model) throw new \Exception(__('tip.order.handleErr').' - stock lock');
                if(empty($is_type)){
                ...
                if(!$goods_sku_model) throw new \Exception(__('tip.order.handleErr').' - stock lock');
                if(empty($is_type)){
    ...
    public function orderSale($goods_id,$num,$is_type=0){
        ...
        try{
            ...
            if(!$goods_model) throw new \Exception(__('tip.order.handleErr').' - sale lock');
            if(empty($is_type)){
    ...
    public function payOrder(){
        ...
        if(!$userInfo['status']) return $this->formatError($userInfo['msg']);
        $userId = $userInfo['data']['id'];

        // 检查支付方式是否传过来
        if(empty($payment_name)) return $this->formatError(__('tip.order.paymentErr'));

        // 如果是余额支付
        $balance = 0;
        if($payment_name == 'balance'){
            if(!Hash::check(request()->pay_password??'',$userInfo['data']['pay_password'])) return $this->formatError(__('tip.pwdErr'));
            $balance = $userInfo['data']['money'];
        }

        // 判断订单号是否为空
        if(empty($order_id)) return $this->formatError(__('tip.order.error').' - pay');
        ...
        $order_model = $this->getService('Order',true);
        // 判断是否存在 指定订单
        if(!$order_model->whereIn('id',$order_arr)->where('user_id',$userId)->exists()) return $this->formatError(__('tip.order.error').' - pay2');
        // 判断是否已经支付过了
        $order_list = $order_model->whereIn('id',$order_arr)->where('user_id',$userId)->where('order_status',1)->get();
        if($order_list->isEmpty()) return $this->formatError(__('tip.order.payed'));
    ...
    protected function createPayOrder($opt = []){
        ...
        if(empty($params['user_id'])) $params['user_id'] = $this->getUserId('users');
        ...
        if($params['recharge_pay']){
            ...
            if($params['payment_name'] == 'money'){
                if($total_price>$params['balance']) return $this->formatError(__('tip.order.moneyNotEnough'));
        ...
        try{
            ...
            if($op){
                if(($op->created_at->timestamp+3)>time()) return $this->formatError(__('tip.order.moneyPay'));
            }
    ...
    public function editOrderStatus($order_id,$order_status,$auth="users"){
        try {
            ...
            if($auth == 'users'){
                ...
                $statusArr = [0.1,4];
                if(!in_array($order_status,$statusArr)) throw new \Exception(__('tip.error'));
                if($order_model->user_id != $userId) throw new \Exception(__('tip.error'));
            }
            ...
            if($auth == 'seller'){
                ...
                $statusArr = [3];
                if(!in_array($order_status,$statusArr)) throw new \Exception(__('tip.error'));
                if($order_model->store_id != $storeId) throw new \Exception(__('tip.error'));

            }

            switch($order_status){
                case 0: // 取消订单
                    if($order_model->order_status != 1) throw new \Exception(__('tip.error'));  // 只有待支付的订单能取消
                ...
                break;
                case 3: // 发货完成 等待 确认收货
                    if($order_model->order_status != 2) throw new \Exception(__('tip.error'));  
                ...
                break;
                case 4: // 确认收货 等待评论
                    if($order_model->order_status != 3) throw new \Exception(__('tip.error'));  // 只有待收货的订单能 确认
                    if(empty($order_model->delivery_no) || empty($order_model->delivery_code))  throw new \Exception(__('tip.error')); 
                break;
                case 5: // 5售后 商家处理
                break;
                case 6: // 6订单完成
    ...
    public function checkAddress(){
        ...
        if(empty($id)) return $this->formatError(__('tip.order.addrErr'));
    ...
    public function createOrderFormat($params){
        ...
        foreach($params['order'] as $v){
            ...
            if(isset($v['collective_id'])) $data['collective_id'] = $v['collective_id']; 
    ...
    public function base64Check(){
        ...
        // 如果为空
        if(empty($base64)) return $this->formatError(__('tip.order.error'));
