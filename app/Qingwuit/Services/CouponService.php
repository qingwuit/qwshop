...
class CouponService extends BaseService{
    ...
    public function receive(){
        ...
        if(empty($coupon_id)) return $this->formatError(__('tip.discount.couponExists').' - 1');
        ...
        try{
            ...
            // 判断优惠券是否失效
            if(now()->gt($coupon_model->end_time)) return $this->formatError(__('tip.failed'));

            // 查看发放总数是否发完
            if($coupon_model->stock<=0) return $this->formatError(__('tip.discount.couponStockErr'));

            $coupon_log_model = $this->getService('CouponLog',true);

            // 先查看是否存在
            if($coupon_log_model->where('coupon_id',$coupon_id)->exists()) return $this->formatError(__('tip.discount.couponHas'));
    ...
    public function use_coupon($coupon_log_id=0,$order_id=0){
        if(empty($coupon_log_id)) return $this->formatError('tip.failed');
        $coupon_log_model = $this->getService('CouponLog',true);
    ...
    public function getCouponByStoreId($store_id){
        ...
        if($list->isEmpty()) return $this->formatError('coupon_empty');
