...
class StoreService extends BaseService{
    ...
    public function createStore($auth = 'users'){
        ...
        if(!empty($userId['belong_id'])) return $this->formatError(__('tip.store.subLimit'));

        $store = $this->getService('Store',true)->where('user_id',$userId)->first();

        if($store) return $this->formatError(__('tip.store.defined'),$store);
    ...
    public function editStore($storeId = null,$whereName='user_id',$auth = 'users'){
        ...
        if(empty($storeId)) return $this->formatError(__('tip.store.notMall'));
        ...
        if($whereName == 'user_id') $storeId = $userId;
        ...
        if($auth != 'admins'){
            ...
            if($store_model->store_verify == 1) $store_verify = 2; //待审核
            if($store_model->store_verify == 3) $store_verify = 2; //审核失败重修改
