...
class AuthController extends Controller
{
    ...
    public function edit(Request $request){
        ...
        if(!isset($data['pay_password']) || empty($data['pay_password']) ) unset($data['pay_password']);
        if(isset($data['pay_password'])) $data['pay_password'] = Hash::make($data['pay_password']);

        // 修改手机号码
        if(isset($data['phone']) && !empty($data['phone']) && $pro == 'User'){
            if($this->getService($pro,true)->whereNotIn('id',[$id])->where('phone',$data['phone'])->exists()) return $this->error(__('tip.phoneExist'));
