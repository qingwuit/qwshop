...
class DashboardController extends Controller
{
    ...
    public function all()
    {
        ...
        $data['today_price'] = $this->getService('Order', true)->where('store_id', $store_id)->where('order_status', '>', 1)->where('pay_time', '>=', date('Y-m-d'))->sum('total_price'); // 今日销售额
        $data['yesterday_price'] = $this->getService('Order', true)->where('store_id', $store_id)->where('order_status', '>', 1)->where('pay_time', '<', date('Y-m-d'))->where('pay_time', '>=', Carbon::yesterday()->format('Y-m-d'))->sum('total_price'); // 昨日销售额
        $data['week_price'] = $this->getService('Order', true)->where('store_id', $store_id)->where('order_status', '>', 1)->where('pay_time', '>=', Carbon::now()->startOfWeek()->format('Y-m-d'))->sum('total_price'); // 这周销售额
        $data['last_week_price'] = $this->getService('Order', true)->where('store_id', $store_id)->where('order_status', '>', 1)->where('pay_time', '<', Carbon::now()->startOfWeek()->format('Y-m-d'))->where('pay_time', '>=', Carbon::now()->subWeeks(1)->startOfWeek()->format('Y-m-d'))->sum('total_price'); // 上周销售额
        $data['month_price'] = $this->getService('Order', true)->where('store_id', $store_id)->where('order_status', '>', 1)->where('pay_time', '>=', Carbon::now()->startOfMonth()->format('Y-m-d'))->sum('total_price'); // 这月销售额
        $data['last_month_price'] = $this->getService('Order', true)->where('store_id', $store_id)->where('order_status', '>', 1)->where('pay_time', '<', Carbon::now()->startOfMonth()->format('Y-m'))->where('pay_time', '>=', Carbon::now()->subMonths(1)->startOfMonth()->format('Y-m'))->sum('total_price'); // 上月销售额
