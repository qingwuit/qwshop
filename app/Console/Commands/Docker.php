...
class Docker extends Command
{
    ...
    public function handle()
    {
        ...
        Artisan::call('passport:keys --ansi'); 
        Artisan::call('storage:link'); 
        
        $domain = $this->anticipate('Please enter your Domain name (https://xxx.com)', ['http://127.0.0.1', 'localhost']);
        
        if (stripos($domain, 'https://')===false && stripos($domain, 'http://')===false && $domain != 'localhost' && $domain != 'localhost' ) {
            $this->error('Domain name error. Please fill in again...');
            return;
        }

        $mysqlHost = $this->anticipate('Please enter your MySQL address (Host)', ['mysql','127.0.0.1', 'localhost']);
